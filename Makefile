...
test_env_up:
	@echo "🧪 Starting up Portal HTTP DB Client test environment ..."
	...
	@sleep 2;
	@echo "⏳ Performing health check on portal-http-db-1 ..."
	@attempts=0; until curl -s http://localhost:8080/healthz >/dev/null || [[ $$attempts -eq 5 ]]; do sleep 2; ((attempts++)); done
	@[[ $$attempts -lt 5 ]] && echo "🖥️  portal-http-db-1 is online ..." || (echo "❌ portal-http-db-1 failed health check" && make test_env_down >/dev/null && exit 1)
	@echo "⏳ Performing health check on portal-http-db-2 ..."
	@attempts=0; until curl -s http://localhost:8081/healthz >/dev/null || [[ $$attempts -eq 5 ]]; do sleep 2; ((attempts++)); done
	@[[ $$attempts -lt 5 ]] && echo "🖥️  portal-http-db-2 is online ..." || (echo "❌ portal-http-db-2 failed health check" && make test_env_down >/dev/null && exit 1)
